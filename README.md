...
const StudentList = ({ students }) => {
    if (students.length === 0) {
        return (
            <p className="w-min mx-auto min-w-max">
                Sorry, no matching students 😴{' '}
            </p>
        );
    } else {
        return students.map((student) => <Student key={student.id} {...student} />);
    }
};
const StateDemo = () => {
    const [state, setState] = useState({
    ...
    const handleToggle = () => {
        setState({
            ...state,
            shouldHideList: !state.shouldHideList,
        });
    };
    const handleSubmit = () => {
        const newStudent = {
        ...
        if (state.name === '' || state.lastName === '') return;
        setState({
            ...state,
            students: [...state.students, newStudent],
            name: '',
    ...
    return (
        <HeaderFooterLayout>
            <main className="py-8">
                ...
                <section className="min-w-80 flex w-full justify-center items-center mt-8">
                    <input
                        value={filterValue}
                    ...
                    <button
                        ...
                        className={`ml-4 rounded-md border-2 border-red-400 text-red-500 py-0.5 px-4 ${
